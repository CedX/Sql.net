...
public static partial class DbConnectionExtensions {
	...
	public static int CountAll<T>(this IDbConnection connection, int timeout = 30, IDbTransaction? transaction = null, SqlCommandBuilder? builder = null) where T: new() {
	...
	/// <returns><see langword="true"/> if the specified entity has been deleted, otherwise <see langword="false"/>.</returns>
	public static async Task<int> CountAllAsync<T>(this IDbConnection connection, int timeout = 30, IDbTransaction? transaction = null, SqlCommandBuilder? builder = null, CancellationToken cancellationToken = default) where T: new() {
		var (command, parameters) = (builder ?? SqlCommandBuilder.Create(connection)).GetCountAllCommand<T>();
		command.Timeout = timeout;
		command.Transaction = transaction;
